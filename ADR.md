...
**Update** (2026-09-10, v1.229.1): follow-up after the first hardening shipped in 1.229.0 — a ~76MB base64 attachment in a composer draft (`session_composer_draft::*`) was outside the cured families and kept crashing startup in background workers. (1) `session_composer_draft` joins both large-payload key lists: Dart `_isLargeCachePayloadPreferenceKey` and the Kotlin `CodeWalkApplication.LARGE_KEY_BASES`; drafts now persist through the hybrid file-backed store (`getSessionComposerDraftJson` / `saveSessionComposerDraftJson`) under the shared 2MB payload ceiling, so a large draft can no longer poison SharedPreferences. (2) The pre-existing `AppConstants.maxFileSize` (10MB) is now enforced in the attachment pipeline (`composerFileFromBytes` for pasted/dropped files and `_resolveAttachmentUrl` for picked files) — it had never been applied, which allowed the 76MB data URL draft. (3) The native cure now logs a scan summary (`entries/candidates/quarantined`) on every process start so logcat silence is distinguishable from a skipped scan.

**Update** (2026-09-11, v1.230.0): device-verified root cause follow-up. Provider catalog caches (16 scopes, ~4.7MB each, ~75MB total) were written directly to SharedPreferences; the startup `getAll` encode OOM'ed. `provider_catalog_cache` now joins both large-payload key lists and catalog get/save routes through the capped hybrid store (oversized catalogs dropped and refetched). `canned_answers` (user data) also routes through the hybrid store under the same ceiling. The native cure additionally quarantines ANY String above 2MB in `FlutterSharedPreferences` regardless of key family (logging key + length, never values); known families keep per-family ceilings (2MB for draft/user-data families, 1MB for regenerable caches). `_GuardedSharedPreferences.setString` now refuses any value above the payload ceiling, making the no-oversized-prefs invariant platform-independent; the legacy migration only removes the preference copy after the file store actually holds the value. Verified on device: boot A quarantined the poisoned catalog, boot B scanned clean, app stayed alive with no OOM.
...
**Note** (issue #160): Virtual History Loading refinement (commit `0c70d0e8`) — decision point 6 is refined under the same official-API ceiling (`limit`-only tail reads, no cursor). Cold open of any session now fetches only a sentinel probe of the newest ~50 messages (window+1) instead of full history, with the extra message making `hasMoreOldMessages` exact at/below the window; top-reach pagination requests resident+chunk(50)+1 and prepends only the slice strictly older than the oldest resident message, falling back to the existing reconciliation merge on anchor miss; unbounded correctness-recovery fetches commit at most the newest 500 resident messages with `hasMoreOldMessages=true` when truncated. Per-session LRU entries and persisted snapshots are capped to the newest initial window (~50 plus pending optimistic messages), and legacy oversized snapshots are bounded on read. The SWR delta tail stays at 200 with unchanged semantics — it may top the resident window up to that tail after revalidation — preserving the delta-tail fallback correctness guarantee; jump-to-first pulls exactly one archived chunk and lands on the new top.
...
## ADR-029: Host-Discovered Quota and Rate-Limit Monitoring for OpenChamber Parity (2026-04-09, updated 2026-09-26)
...
2. **Strategy-Chain Transport** — Quota data is fetched using a tiered discovery strategy:
    ...
    - **Hidden Shell Fallback** — Create a hidden ephemeral OpenCode session, execute a Base64-encoded Node.js probe through `POST /session/:id/shell`, parse the final `CW_QUOTA_JSON:` line, and delete the probe session after completion. Bind each probe to the selected host/base-URL chain; if the selected server changes before cleanup, skip cleanup rather than sending the old session ID to the newly selected host, following the existing host-bound file-operations pattern.
3. **Popup, Full Desktop Utility-Pane Mirror, and Polling Lifecycle** — Compact/mobile layouts stay popup-only: the monitoring interface is restricted to the "Context usage" popup, hidden by default to preserve composer real-estate, appearing on open or explicit manual refresh; the popup never auto-polls and shares the normal `QuotaProvider` snapshot/cache. On large desktop only, the utility pane mirrors the **full Context usage section**: metrics grid, compaction explanation, safely gated compact action (hidden unless eligible and never calls `Navigator.pop`), and quota/rate-limit section, reusing the same `QuotaProvider`/TTL as the popup (no second fetch). Only the visible desktop utility pane auto-polls, at a 20-minute interval, while its route is current, the app is resumed or inactive, an active server is selected, and the pane is visible and not covered by a maximized terminal. Stop polling when the route or pane is hidden, the app is hidden/paused/detached, or the section is disposed. The pane's eager bounded-scroll mounting is not pixel visibility. Revealing it performs a normal TTL-governed load (60-second shared TTL); manual refresh may force a load.
4. **Grouped Providers with Provider-Specific Window Semantics** — UI displays providers grouped by parent organization (OpenAI, Anthropic, etc.) using progress bars for usage and remaining quota. For Codex, use actual window durations to label REST and shell results and preserve credits as credits; when duration is unknown, show a neutral unknown label and do not infer duration or Pace. Other providers retain existing recognized-label mapping and inference. xAI Pace is available only when a valid current-period start/end pair establishes both duration and reset at the same end. If an explicit current period is invalid, do not synthesize a reset or Pace; legacy/incomplete metadata may show usage only with unknown/neutral period semantics.
...
**Amendment** (issue #199, commits `8bdbf89` + `7f95449`): decision 3 large-desktop mirror extended from the read-only `QuotaPopupSection` to the FULL context-usage section (metrics grid, compaction explanation, safely-gated compact action hidden unless eligible, never `Navigator.pop`).

**Amendment** (G1 v1.256.0, 2026-09-26; code commits `b20ea173` and `bbe22e2c`; release commit `f470921b`): records the desktop-only bounded polling lifecycle and evidence-bounded Codex/xAI window semantics above. xAI's public descriptor and official grok-build documentation are secondary provider-API references, not part of the official OpenCode protocol. No new OpenCode server API or client-owned provider credentials are introduced.
...
- **Evidence-bounded Pace** — Codex uses reported duration when available and does not infer Pace for unknown-duration windows; xAI Pace requires valid current-period bounds. Other providers retain their existing recognized-label inference.
...
- `lib/presentation/widgets/quota/quota_popup_section.dart` — Root quota widget embedded in the Context usage popup; owns popup load/manual-refresh behavior and desktop utility-pane polling lifecycle
...
This feature is compliant with ADR-023. Official OpenCode remains the primary source for all agent contracts and core app behavior. OpenChamber is used exclusively as an optional parity source for the quota/rate-limit feature. In the absence of OpenChamber REST endpoints, the app falls back to a hidden ephemeral shell probe without PTY process lifecycle changes, ensuring no divergence from official server capabilities. The xAI descriptor and official grok-build documentation are secondary public provider-API references only; they do not extend or redefine the official OpenCode protocol. Provider credentials remain host-owned, and this amendment adds no OpenCode endpoint or server API.
