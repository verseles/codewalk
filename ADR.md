...
3. **Popup-Only UI (Compact-First), Large-Desktop Mirror** — Compact/mobile layouts stay popup-only: the monitoring interface is restricted to the "Context usage" popup, hidden by default to preserve composer real-estate, appearing only on explicit user invocation. On large-desktop only, the desktop utility pane (`_buildDesktopUtilityPane`) is permitted to mirror the FULL context-usage section — metrics grid, compaction explanation, safely-gated compact action (hidden unless eligible, never `Navigator.pop`), plus the read-only quota/rate-limit section — by reusing `QuotaPopupSection` with the same `QuotaProvider`/TTL as the Context usage popover (no second fetch). The popover itself is unchanged.
