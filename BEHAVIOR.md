...
- **Then** regular tabs size per title between fixed minima and a fixed maximum (317.2px desktop / 278.2px compact, or the viewport width when narrower): the selected tab (with the context-usage knob) floors at 150px desktop / 140px compact, while inactive tabs (no knob, so no reserved knob space) floor at 120px / 100px; short titles shrink to the applicable floor, long titles grow up to the maximum and ellipsize, and tabs never compress below their floor — overflow scrolls horizontally
...
- **Then** the selected pinned tab expands to its title-sized content width between the same knob-bearing minimum (150px desktop / 140px compact) and maximum, and the pinned region cap grows to fit it so the expanded tab is fully visible without horizontal scrolling, while regular tabs keep at least their minimum usable region width
...
- **Then** the strip supports horizontal overflow, ensures the selected tab is visible, exposes the full title in a tooltip and semantics, reports selected and busy/retry status semantics, and re-measures tab widths from their titles when a title or text scale changes, keeping tabs responsive to available space
...
- **Then** the same settlement applies when the final assistant content completes in place or the session status settles to idle on the current session: the composer status clears within its hide delay without requiring a tab, project, or screen switch, while genuine tool-only busy turns keep showing progress
