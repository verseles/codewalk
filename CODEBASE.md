...
- Enabled selectable message content for both assistant and user messages (`SelectionArea` + `MarkdownBody.selectable`), removed inline `Copy` buttons, and changed full-message copy to trigger only from double-tap/double-click on bubble background (text double-click keeps native word selection).
