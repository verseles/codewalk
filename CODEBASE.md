...
lib/presentation/pages/settings/sections/speech_settings_section.dart # Speech settings UI: STT engine/model controls plus read-aloud provider selection, secure API-key entry, and voice/model/base-URL fields; includes cloud API STT section (engine entry, provider/base-URL/model fields, per-provider API-key management through `SttApiKeyStorage` with saved/removed/unavailable status) 
