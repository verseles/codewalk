...
lib/presentation/pages/settings/sections/speech_settings_section.dart # Speech-to-text settings UI only: STT engine/model controls, persisted keep-in-memory switch, offline model management (subscribes to `SttModelDownloadTracker`, persists settings after route disposal), silence timeout, and cloud STT API configuration with per-provider API-key management through `SttApiKeyStorage` (saved/removed/unavailable status)
...
test/widget/speech_settings_retention_test.dart # Speech settings keep-in-memory switch widget coverage
