...
lib/data/datasources/quota_remote_datasource.dart # Strategy-chain quota discovery: tries OpenChamber REST (`GET /api/quota/providers`) then falls back to a hidden ephemeral shell probe (`CW_QUOTA_JSON:`) for vanilla OpenCode hosts; OpenCode Go is probed through the host's `auth.json` Bearer key (issue #96); xAI is probed through host `auth.json` OAuth
  └── quota_remote_datasource.part.js.dart # JS payload generation part file: shared helpers + shell probes for Claude, OpenRouter, Codex, Google, GitHub Copilot, OpenCode Go, NanoGPT, Wafer, Kimi, ZhipuAI, MiniMax, MiniMax CN, z.ai, Cursor, Ollama Cloud, xAI, DeepSeek, and ClinePass; the OpenCode Go probe reads the `opencode-go` Bearer key (`key`/`access`/`token`) from the host `auth.json` and calls `GET https://opencode.ai/zen/go/v1/usage` (rolling/weekly/monthly windows, 15s abort timeout, typed error codes `authentication`/`invalid_response`/`request_failed`); the xAI probe (`fXai`/`parseXaiUsage`) reads host `auth.json` OAuth (`xai`/`grok`/`x-ai`, `type=oauth`), calls `https://grok.com/grok_api_v2.GrokBuildBilling/GetGrokCreditsConfig`, returns billing_cycle usedPercent/resetAt, skips API-key entries, and may persist refreshed OAuth tokens on the host; the DeepSeek probe (`fDeepseek`) reads the host `auth.json` `deepseek` key/token and calls `GET https://api.deepseek.com/user/balance` (credits_balance USD then CNY); the ClinePass probe (`fClinePass`) reads the host `auth.json` `cline-pass` key/token and calls `GET https://api.cline.bot/api/v1/users/me/plan/usage-limits` (5h/weekly/monthly windows); the legacy workspace-id/auth-cookie dashboard flow is removed (issue #96); `_supportedAuthKeys` still recognizes Snowflake Cortex and Cohere North aliases for diagnostics (xAI is no longer aliases-only)
...
lib/presentation/providers/quota_provider.dart # Host-discovered quota state: polls `QuotaRemoteDataSource`, TTL-based cache (60s) scoped per `serverId`, normalises raw data into `QuotaProviderGroup` list ordered by severity; `ensureLoaded()` for lazy UI-triggered fetch with server-switch stale-discard guard (discards late results after server change and refetches for the current server); Codex single-window label preserved using provider name instead of raw API label (guarded by `result.providerId != 'codex'`); exposes `hasOpenCodeGoFailure`/`openCodeGoErrorCode` from the OpenCode Go result and runs a one-shot best-effort `clearOpenCodeGoDashboardCredentials()` purge of legacy dashboard secrets before fetching (issue #96)
...
lib/presentation/services/stt_model_archive_installer_io.dart # Shared on-demand STT model archive installer: streams archives to disk and extracts off the UI isolate; used by Moonshine/Parakeet/SenseVoice/Nemotron managers
lib/presentation/services/stt_model_download_tracker.dart # App-lifetime single-active on-device model progress/results (`SttModelDownload` active/lastFinished); five IO model managers wrap downloads in `run()`; consumed by speech settings, Nemotron dialog, and app shell root snackbar
...
lib/presentation/pages/settings/sections/speech_settings_section.dart # Speech-to-text settings UI only: STT engine/model controls, offline model management (subscribes to `SttModelDownloadTracker`, persists settings after route disposal), silence timeout, and cloud STT API configuration with per-provider API-key management through `SttApiKeyStorage` (saved/removed/unavailable status)
