...
Refined the sidebar navigation style to match area semantics: removed explicit `Chat` action and kept a compact one-line `Areas` row with actionable `Logs` and `Settings` buttons only.
