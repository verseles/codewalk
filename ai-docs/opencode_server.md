...
| `POST` | `/session/:id/command` | Execute a slash command | body: `{ messageID?, agent?, model?: string (provider/model), command: string, arguments: string }`, returns `{ info:`[Message](https://github.com/anomalyco/opencode/blob/dev/packages/sdk/js/src/gen/types.gen.ts)`, parts:`[Part[]](https://github.com/anomalyco/opencode/blob/dev/packages/sdk/js/src/gen/types.gen.ts)`}` |
