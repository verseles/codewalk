...
@visibleForTesting
bool senseVoiceChunkHasSpeech(Float32List samples, {double threshold = 0.015}) {
  if (samples.isEmpty) {
    return false;
  }
  var sum = 0.0;
...
class SenseVoiceSpeechInputService implements SpeechInputService {
  ...
  Future<bool> initialize() async {
    ...
    try {
      ...
      AppLogger.error(
        'SenseVoice bindings initialization failed',
        error: error,
  ...
  Future<void> startListening({
    ...
    try {
      ...
      AppLogger.error(
        'SenseVoice recognizer initialization failed',
        error: error,
        stackTrace: stackTrace,
      );
      _isAvailable = false;
    ...
    Future<void> finishSession() async {
      ...
      if (utterance.isNotEmpty) {
        ...
        try {
          ...
          AppLogger.error(
            'SenseVoice offline decode failed',
            error: error,
            stackTrace: stackTrace,
          );
          onError();
    ...
    try {
      ...
      AppLogger.error(
        'SenseVoice recorder stream start failed',
        error: error,
        stackTrace: stackTrace,
      );
      _isListening = false;
    ...
    _audioSub = audioStream.listen(
      (chunk) {
        ...
        buffer.add(samples);
        if (senseVoiceChunkHasSpeech(samples)) {
          armSilenceTimer();
        }
      },
      onError: (error) {
